...
const getTripController = async (req, res) => {
  try {
    ...
    if (!start || !end || !date) {
      return res
        .status(400)
        .json({ error: "Start, end locations and date are required" });
    }
...
const bookSeatsController = async (req, res) => {
  try {
    ...
    const statusCode = error.message.includes("UNIQUE constraint failed")
      ? 409
      : 500;
...
const getBookedSeatsController = async (req, res) => {
  try {
    ...
    const bookedSeats = await SeatBooking.find({ tripId: tripId }).lean();
...
const getTripsController = async (req, res) => {
    console.log("getTripsController");  
  try {
...
// get trip by busRoute
const getTripByBusRoute = async (req, res) => {
  try {
    const { busRoute } = req.params;
    const trip = await Trip.find({ busRoute });
