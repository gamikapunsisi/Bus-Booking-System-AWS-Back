...
const createTrip = async (bus_route, driver_name, conductor_name, trip_date, departureTime, arrivalTime) => {
  ...
  const trip = await Trip.create({
    tripId: trip_id,
    busRoute: bus_route, // Ensure this is an ObjectId, not a string
    departureTime: departureTime,
    arrivalTime: arrivalTime,
    tripDate: trip_date,
    seatStatus: bus.seatLayout,
    fare: fare,
    fromCity: fromCity, // This is fine as it's defined as a String
    toCity: toCity,     // This is fine as it's defined as a String
    busId: bus_id,      // Ensure this is an ObjectId, not a string
  });
